...
void loop(){
  ...
  if(Serial.available() > 0){
    ...
    for(vibration_counter = 0; vibration_counter < 625; vibration_counter++){           //Do this loop 625 times
      delay(3);                                                                         //Wait 3000us.
      esc_1 = 1000;                                                                     //Set the pulse for ESC 1 - 4 to 1000us.
      esc_2 = 1000;                                                                     
      esc_3 = 1000;                                                                     
      esc_4 = 1000;                                                                     
  ...
  if(data == 'r'){
    ...
    receiver_input_channel_1 = convert_receiver_channel(1);                           //Convert the actual receiver signals for pitch, roll, throttle, yaw to the standard 1000 - 2000us.
...
void esc_pulse_output(){
  ...
  timer_channel_1 = esc_1 + zero_timer;                          //Calculate the time when digital port 4 - 7 is set low.
  ...
  while(PORTD >= 16){                                            
    ...
    if(timer_channel_1 <= esc_loop_timer)PORTD &= B11101111;     //When the delay time is expired, digital port 4 - 7 is set low.
...
void gyro_signalen(){
  ...
  if(eeprom_data[31] == 1){
    ...
    acc_axis[1] = Wire.read()<<8|Wire.read();                    //Add the low and high byte to the acc_x, acc_y,acc_z variable.
