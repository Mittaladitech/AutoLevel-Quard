...
void setup(){
  ...
  PCMSK0 |= (1 << PCINT0);  // set PCINT0 (digital input 8 to 11) to trigger an interrupt on state change
  PCMSK0 |= (1 << PCINT1);  
  PCMSK0 |= (1 << PCINT2); 
  PCMSK0 |= (1 << PCINT3);  
...
void loop(){
  ...
  if(error == 0){
    ...
    Serial.println(F("You can now calibrate the esc's and upload the Step2 code."));
  }
  else{
   Serial.println(F("The setup is aborted due to an error."));
   Serial.println(F("Check the Q and A page of the ***********************************************************-AL project on:"));
...
void intro(){
  ...
  delay(1000);
  Serial.println(F(""));
  Serial.println(F("Step1 Setup Program"));
  Serial.println(F(""));
  Serial.println(F("***********************************************************"));
  delay(1500);
 // Serial.println(F("www.brokking.net"));
  Serial.println(F(""));
